...
if ! command -v openssl >/dev/null 2>&1; then
  echo "openssl is required to generate Tuple secrets." >&2
  exit 1
fi
...
APP_NAME="${FLY_APP_NAME:-tuple-$(openssl rand -hex 3)}"
POSTGRES_APP_NAME="${FLY_POSTGRES_APP_NAME:-${APP_NAME}-db}"
REGION="${FLY_REGION:-cdg}"
ORG="${FLY_ORG:-personal}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$(mktemp "${TMPDIR:-/tmp}/tuple-fly.XXXXXX.toml")"
...
echo "Setting Tuple secrets..."
tuple_secret_args=(--app "$APP_NAME" --stage)

if secret_exists "BETTER_AUTH_SECRET"; then
  echo "BETTER_AUTH_SECRET already exists; leaving it unchanged."
else
  tuple_secret_args+=("BETTER_AUTH_SECRET=$(openssl rand -hex 32)")
fi

if secret_exists "TUPLE_ENCRYPTION_KEY"; then
  echo "TUPLE_ENCRYPTION_KEY already exists; leaving it unchanged."
else
  tuple_secret_args+=("TUPLE_ENCRYPTION_KEY=$(openssl rand -hex 32)")
fi

if [ "${#tuple_secret_args[@]}" -gt 3 ]; then
  fly secrets set "${tuple_secret_args[@]}"
else
  echo "Tuple secrets already exist."
fi

echo "Deploying Tuple..."
fly deploy --app "$APP_NAME" --config "$CONFIG_FILE"

echo
echo "Tuple URL: https://${APP_NAME}.fly.dev"
