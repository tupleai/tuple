...
export function oauthDoneHtml(success: boolean, nonce?: string, providerLabel = 'Login'): string {
  const message = success ? 'tuple-oauth-success' : 'tuple-oauth-error';
  const text = success
    ? 'Login successful!'
  ...
  return `<!DOCTYPE html>
<html>
<head><title>Tuple — ${providerLabel}</title></head>
...
<script${nonceAttr}>
try{var bc=new BroadcastChannel('tuple-oauth');bc.postMessage({type:'${message}'});bc.close();}catch(e){}
