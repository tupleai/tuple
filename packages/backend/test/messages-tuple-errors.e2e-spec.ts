/**
 * The Messages log must show every Tuple-authored failure.
 *
...
const SETUP_ERROR = '[↗ Tuple M100] No anthropic API key yet.';
const BAD_REQUEST_ERROR = '[↗ Tuple M300] `messages` array is required.';
...
describe('Tuple-authored errors in the Messages log', () => {
  ...
  it('narrows to every Tuple origin at once with origin=tuple', async () => {
    const res = await list('&origin=tuple');
    const ids = res.body.items.map((i: { id: string }) => i.id);
