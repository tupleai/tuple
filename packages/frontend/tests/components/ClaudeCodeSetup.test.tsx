...
describe("ClaudeCodeSetup", () => {
  ...
  it("renders the JSON settings block with Tuple auto model and ANTHROPIC vars", () => {
    const { container } = render(() => <ClaudeCodeSetup {...baseProps} />);
  ...
  it("masks the API key when only a prefix is available", () => {
    const { container } = render(() => (
      <ClaudeCodeSetup {...baseProps} keyPrefix="tuple_abc" />
    ));
  ...
  it("uses 'tuple_YOUR_KEY' placeholder when neither full key nor prefix is set", () => {
    const { container } = render(() => <ClaudeCodeSetup {...baseProps} />);
  ...
  it("toggles the API key reveal when the eye button is clicked", () => {
    const { container } = render(() => (
      <ClaudeCodeSetup {...baseProps} apiKey="tuple_full_secret_value" keyPrefix="tuple_full" />
    ));
    // Masked by default.
